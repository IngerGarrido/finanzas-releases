...
if ($method === 'POST') {
    ...
    $db->prepare("INSERT INTO gastos_pendientes (usuario_id, persona, descripcion, monto_total, fecha_limite) VALUES (?,?,?,?,?)")
       ->execute([$uid, trim($b['persona']), trim($b['descripcion'] ?? ''), (float)$b['monto_total'], $b['fecha_limite'] ?: null]);
    ok(['id' => (int)$db->lastInsertId()], 201);
}
...
if ($method === 'PUT') {
    ...
    if ($tipo === 'pago') {
        ...
        $db->prepare("UPDATE gastos_pendientes SET persona=?, descripcion=?, monto_total=?, fecha_limite=?
                      WHERE id=? AND usuario_id=?")
           ->execute([trim($b['persona']), trim($b['descripcion'] ?? ''), (float)$b['monto_total'], $b['fecha_limite'] ?: null, $id, $uid]);
    }
    ok();
}
